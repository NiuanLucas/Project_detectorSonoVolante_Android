...
public abstract class MLVideoHelperActivity extends AppCompatActivity {
    ...
    void bindPreview(@NonNull ProcessCameraProvider cameraProvider) {
        ...
        imageAnalysis = new ImageAnalysis.Builder().setTargetAspectRatio(AspectRatio.RATIO_4_3).build();
    ...
    public void switchCamera() {
        ...
        // Re-bind the camera with the new setting
        bindCameraUseCases();
    }

    private void bindCameraUseCases() {
        // Adiciona um listener ao cameraProviderFuture para obter o ProcessCameraProvider
        // de forma assíncrona e segura, tratando exceções potenciais.
        cameraProviderFuture.addListener(() -> {
            try {
                // Tenta obter o ProcessCameraProvider. Isso pode lançar uma ExecutionException
                // se houver um problema ao iniciar a câmera, ou uma InterruptedException
                // se a thread atual for interrompida enquanto espera.
                ProcessCameraProvider cameraProvider = cameraProviderFuture.get();

                // Desvincula todos os casos de uso antes de reassociá-los
                cameraProvider.unbindAll();

                // Vincula os casos de uso da câmera (preview, análise de imagem, etc.)
                bindPreview(cameraProvider);
            } catch (ExecutionException e) {
                // Trata a exceção causada por um erro durante a execução
                Log.e("MLVideoHelperActivity", "Erro ao obter o ProcessCameraProvider.", e);
            } catch (InterruptedException e) {
                // Trata a exceção causada pela interrupção da thread de espera
                Log.e("MLVideoHelperActivity", "Thread interrompida ao esperar o ProcessCameraProvider.", e);
